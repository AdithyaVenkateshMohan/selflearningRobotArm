...
def moveAndCheck(angles):
    ...
    red = detect_w_avg(cap, 600,0,100,100,189,255,255, "red")
    print("red", red)
    pixel_position.append(red)
...
def train_neural_networks(x):
    ...
    cp = (predict-y)**2
    cost = (cp[0,0]+cp[0,1])**(0.5) # distance bettween the desired and actucal pt in top view + front view is the cost
    ...
    with tf.Session() as sess:
        ...
        # code for coverting the data to batch wise and giving it to the neural network
            ...
            test_y,test_x=moveRandom()
            cr = (predict-y)**2
            accuracy = (cr[0,0]+cr[0,1])**(0.5)
