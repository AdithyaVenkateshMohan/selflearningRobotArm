...
#     return angles,pixel_position # removed

#removed move random converged to one function
...
def Test_Neural_Network_model(wat) :
    ...
    while i< no_nodes_hl1:
        ...
        layer1[:,i]= (np.dot(test_x, hidden_Layer_1[:,i] ))
        i=i+1
...
while i<5:
    rand = np.random.rand(3)
    t_y = rand * 180
    t_x=moveAndCheck(t_y)
    train_x=np.append(train_x,t_x,axis=0)
    print("seee the t_x", train_x)
...
n =len(train_y)
print("seee the t_x",train_x)
train_x = np.reshape(train_x,(int(m/2),2))

train_y = np.reshape(train_y,(int(n/3),3))

x=tf.placeholder('float',[None,len(train_x[0])])

y=tf.placeholder('float')
print("training x",train_x,"train_y",train_y,"len",len(train_x[0]))
...
def train_neural_networks(x):
    ...
    cp = (moveAndCheck(predict*180)-y)**2
    cost = (cp[0]+cp[1])**(0.5) # distance bettween the desired and actucal pt in top view + front view is the cost
    ...
    with tf.Session() as sess:
        ...
        # code for coverting the data to batch wise and giving it to the neural network
            ...
            # rand = np.random.rand(3)
            # test_y = rand * 180
            # test_x=moveAndCheck(test_y)
            cr = (predict-y)**2
            accuracy = (cr[0]+cr[0])**(0.5)
            print("the accuracy: ",accuracy.eval({x: test_x , y : test_x}))
